...
var ENDPOINT;
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2018-11-30T04-09-35.json?1543551029166'; // Otoño 2018
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2018-10-13T03-33-18.json?1540439461840'; // Primavera 2018
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2019-03-04T17-21-11.json?1551720799377'; // Otoño 2019
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2019-07-20T01-10-25.json?v=1563585375135'; // Primavera 2019
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2020-03-17T21-17-49.json?v=1584479931413'; // Otoño 2020
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2020-08-18T02-33-32.json?v=1597718060240'; // Primavera 2020
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2021-01-26T01-33-44.json?v=1611624866202'; //Otoño 2021
//ENDPOINT = 'https://cdn.glitch.com/5c86d7f5-cdbd-4acb-980c-620ba3e55c14%2F2021-07-29T00-43-02.json?v=1627519494089'; //Primavera 2021
//ENDPOINT =  'https://cdn.glitch.global/5c86d7f5-cdbd-4acb-980c-620ba3e55c14/2022-01-05T22-42-03.json?v=1641422559340'; // Otoño 2022
//ENDPOINT = 'https://cdn.glitch.global/5c86d7f5-cdbd-4acb-980c-620ba3e55c14/2022-06-01T23-40-29.json?v=1654126928934'; // Primavera 2022
...
function checkAvailability(horario) {
  let idsHorario = horario
    ...
    .flat(); // array de arrays que es combinado en uno solo con .flat()
  // Se chequea que cada td contenga menos de 2 divs.
  return idsHorario.every(
    (id) => document.getElementById(id).childElementCount < 2
  );
}

// Obtiene la fecha y hora de la última actualización del data set de cursos desde la URL.
function getLastUpdated(endpoint, utc) {
  // Regular expression to find date-time pattern in file name.
  let regExp = /(\d+-\d+-\d+)T(\d+-\d+-\d+)/;
  let fechaHora = regExp.exec(ENDPOINT);
  let fecha = fechaHora[1].split("-");
  let year = fecha[0];
  let month = fecha[1];
  let day = parseInt(fecha[2]);
  let time = fechaHora[2].split("-");
  let hour = parseInt(time[0]);
  let minutes = time[1];
  let seconds = time[2];
  // Vemos si horas es menor al tiempo universal coordinado (UTC).
  if (hour < Math.abs(utc)) {
    // Restamos 1 día, ya que eran antes de las 24hs
    day -= 1;
    // Obtenemos la hora. Ejemplo la hora es 1, y utc: -4, la hora de la consulta fue: 21
    hour = 24 + hour + utc;
  } else {
    hour -= Math.abs(utc);
  }
  hour = hour.toString().padStart(2, 0); //format the number with 2 digits (e.g, "01").
  day = day.toString().padStart(2, 0);
  return `Última actualización: <br> ${day}-${month}-${year} ${hour}:${minutes}:${seconds}`;
}
...
const handlers = {
  ...
  compartirEnWhatsapp: () => {
    ...
    let urlencodedtext = encodeURIComponent(
      `Simula tu toma de ramos de ${periodo} en https://hurario.glitch.me/`
    );
